...
export default function Settings() {
  ...
  const loadAllSettings = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showNotification({ message: "Settings loaded successfully", type: "success" });
      } else {
        showNotification({ message: result.error || "Failed to load settings", type: "error" });
      }
    } catch (error) {
      console.error("Error loading settings:", error);
      showNotification({ message: "Failed to connect to server", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveAllSettings = async () => {
    const selectedAssessmentCount = Object.values(assessmentTypes).filter(v => v === true).length;
    if (selectedAssessmentCount > 2) {
      showNotification({ message: "You can only select up to 2 assessment types!", type: "error" });
      return;
    }
    ...
    try {
      ...
      if (generalResult.success && configResult.success) {
        ...
        showNotification({ message: "All settings saved successfully!", type: "success" });
        await loadAllSettings();
      } else {
        showNotification({ message: "Failed to save some settings", type: "error" });
      }
    } catch (error) {
      console.error("Error saving settings:", error);
      showNotification({ message: "Failed to save settings", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveGeneralSettings = async () => {
    ...
    try {
      ...
      if (result.success) {
        showNotification({ message: "General settings saved!", type: "success" });
      } else {
  ...
  const saveLoginConfig = async () => {
    ...
    try {
      ...
      if (result.success) {
        showNotification({ message: "Login configuration saved!", type: "success" });
      } else {
        showNotification({ message: result.error || "Failed to save", type: "error" });
      }
    } catch (error) {
  ...
  const saveSchoolTypes = async () => {
    ...
    try {
      ...
      if (result.success) {
        showNotification({ message: "School types saved!", type: "success" });
      } else {
        showNotification({ message: result.error || "Failed to save", type: "error" });
      }
    } catch (error) {
  ...
  const saveAssessmentTypes = async () => {
    ...
    try {
      ...
      if (result.success) {
        showNotification({ message: "Assessment types saved!", type: "success" });
      } else {
        showNotification({ message: result.error || "Failed to save", type: "error" });
      }
    } catch (error) {
  ...
  const resetAllSettings = async () => {
    if (window.confirm("Are you sure you want to reset all settings to defaults? This cannot be undone.")) {
      ...
      try {
        ...
        if (result.success) {
          showNotification({ message: "All settings reset to defaults!", type: "success" });
          await loadAllSettings();
        } else {
          showNotification({ message: result.error || "Failed to reset", type: "error" });
        }
      } catch (error) {
        console.error("Error resetting settings:", error);
        showNotification({ message: "Failed to reset settings", type: "error" });
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleModeChange = (newMode) => {
    ...
    localStorage.setItem("system_mode", newMode);
    showNotification({ message: `Switched to ${newMode} mode`, type: "success" });
  };

  const handleLiveModeChange = (newLiveMode) => {
    setLiveMode(newLiveMode);
    localStorage.setItem("live_mode", newLiveMode);
    showNotification({ message: newLiveMode ? "Live mode enabled" : "Live mode disabled", type: "success" });
  };

  const handleSyncComplete = (newSyncTime) => {
    setLastSync(newSyncTime);
    showNotification({ message: "Sync completed successfully!", type: "success" });
  };

  const handleDeactivate = () => {
    if (window.confirm("Are you sure you want to deactivate this software?")) {
      setActivation({ ...activation, isActivated: false });
      showNotification({ message: "Software deactivated", type: "warning" });
    }
  };
