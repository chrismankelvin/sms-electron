// MiniSettingsPage.jsx - Converted to use IPC communication
...
// Import services using IPC
import { syncData, checkBackendHealth, getAllMiniSettings,updateTheme,updateSchoolType,saveAllMiniSettings,setSeenMiniSettings,updateScreensaver } from "../../services/api.service";
...
export default function MiniSettingsPage({ onComplete }) {
  ...
  useEffect(() => {
    // Load settings from JSON file via IPC
  ...
  // Load settings from JSON file using IPC
  const loadSettingsFromJson = async () => {
    try {
      const settings = await getAllMiniSettings();        //done
  ...
  // Toggle theme and save to JSON via IPC
  ...
  // Toggle screensaver and save to JSON via IPC
  const handleScreensaver = async (enabled) => {
    ...
    // Save to JSON file via IPC
    try {
      await updateScreensaver(enabled);
    } catch (error) {
  ...
  // Select school type and save to JSON via IPC
  const handleSchoolType = async (type) => {
    ...
    // Save to JSON file via IPC
    try {
      await updateSchoolType(type);              //done
    } catch (error) {
  ...
  // SINGLE COMPLETE SYNC FUNCTION using IPC
  const performCompleteSync = async () => {
    try {
      ...
      // Test if backend is running via IPC
      try {
      ...
      // Make ONE call to sync/complete via IPC
      setSyncMessage("Syncing all data to cloud...");
      
      const result = await syncData({
       sync_school: true,        // Matches your backend's sync_school field
  sync_activation: true,    // Matches your backend's sync_activation field
  sync_devices: true,       // Matches your backend's sync_devices field
  device_batch_size: 20     // Matches your backend's device_batch_size field
      });
  ...
  const handleSaveSettings = async () => {
    ...
    // Save ALL mini settings to JSON file via IPC
    try {
      await saveAllMiniSettings(allMiniSettings);         //done
      console.log("✅ Mini settings saved to JSON file successfully via IPC");
    } catch (error) {
    ...
    try {
      await setSeenMiniSettings();              //done
    } catch (error) {
      console.error("Failed to mark as seen in JSON:", error);
      localStorage.setItem("hasSeenMiniSettings", "true");
    }
    
    // Start sync using IPC
    setIsSyncing(true);
    setSyncError(null);
    setSyncResult(null);
  ...
  const handleContinueAnyway = () => {
    // Mark that user has seen mini-settings in JSON file via IPC
    const markAsSeen = async () => {
      try {
        await setSeenMiniSettings();
      } catch (error) {
