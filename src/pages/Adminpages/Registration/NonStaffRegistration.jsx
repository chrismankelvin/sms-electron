...
const NonStaffRegistration = ({ onSuccess, onError }) => {
  ...
  const fillTestData = () => {
    ...
    const roles = ['Administrative Assistant', 'Security Guard', 'Cleaner', 'Librarian', 'Clerk', 'Driver', 'Secretary', 'Storekeeper'];
    const departments = ['Administration', 'Security', 'Maintenance', 'Library', 'Records', 'Transport', 'Finance'];
    const randomRole = roles[Math.floor(Math.random() * roles.length)];
    const randomDept = departments[Math.floor(Math.random() * departments.length)];
    
    const testNonStaff = {
      // Personal Information
      first_name: `NonStaff${randomNum}`,
      surname: `Worker${randomNum}`,
      other_names: 'Test',
      ...
      email_address: `test.nonstaff.${timestamp}@example.com`,
      ...
      next_of_kin_contact: '0244123460',
      
      // Emergency
      emergency_contact_one: '0244123461',
      emergency_contact_two: '0244123462',
    };
    
    setFormData(testNonStaff);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Submitting Non-Teaching Staff registration data:', registrationData);
      ...
      if (result && result.success) {
        showNotification('Non-Teaching Staff registered successfully!', 'success');
        
        // Show success message with non-staff details
        setTimeout(() => {
          alert(`Non-Teaching Staff registered successfully!\n\nStaff Number: ${result.non_staff_number || result.staff_number}\nUsername: ${result.username}\nDefault Password: ${result.default_password}\n\nPlease provide these credentials to the staff member.`);
        }, 500);
