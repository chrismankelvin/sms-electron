...
import { useState, useEffect } from 'react';
import { BookOpen, Download, Printer, Search, Filter, X, Trophy, Award, Loader, RefreshCw, AlertCircle } from 'lucide-react';
import '../../../styles/subject-results.css';
...
const subjectResultsService = {
  ...
  async getFilterOptions(classId = null) {
    let url = `${API_BASE_URL}/subject-results/options`;
    if (classId) {
...
function SubjectResults() {
  ...
  const loadFilterOptions = async () => {
    try {
      setLoading(true);
      // First load only terms and classes (without subjects)
      const data = await subjectResultsService.getFilterOptions();
      setTerms(data.terms);
      setClasses(data.classes);
      ...
      if (data.classes.length > 0) {
        const defaultClassId = data.classes[0].id.toString();
        setSelectedClass(defaultClassId);
        // Subjects will be loaded by the useEffect that watches selectedClass
      }
    } catch (error) {
      showAlert('Failed to load filter options: ' + error.message, 'error');
    } finally {
  ...
  const loadSubjectsForClass = async () => {
    ...
    try {
      ...
      const data = await subjectResultsService.getFilterOptions(selectedClass);
      setSubjects(data.subjects);
      ...
      if (data.subjects.length > 0) {
        ...
        setSelectedSubject('');
        showAlert('No subjects assigned to this class', 'warning');
      }
    } catch (error) {
  ...
  if (loading && results.length === 0 && !loadingSubjects) {
    return (
      <div className="subject-results-container">
        <div className="loading-container">
          <Loader size={48} className="spinner" />
    ...
    <div className="subject-results-container">
      ...
      <div className="filter-bar" style={{ marginBottom: '1rem', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <div className="filter-group">
          ...
          {loadingSubjects && <Loader size={14} className="spinner" style={{ marginTop: '0.25rem' }} />}
          {!loadingSubjects && subjects.length === 0 && selectedClass && (
        ...
        {results.length === 0 && !loading ? (
          <div className="empty-state">
            ...
            {selectedClass && subjects.length === 0 && (
              <p className="hint-text">Please assign subjects to this class first</p>
            )}
          </div>
        ) : (
          <table className="academic-years-table">
