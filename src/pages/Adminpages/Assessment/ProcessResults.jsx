      ...
      {step === 1 && (
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div className="form-group">
              <label>Subject (Optional - Leave blank for all subjects in this class)</label>
              <select 
                className="form-select" 
                value={selectedSubject} 
                onChange={(e) => setSelectedSubject(e.target.value)}
                disabled={processing || classSubjects.length === 0}
              >
                <option value="">All Subjects in {getClassName()}</option>
                {classSubjects.map(subj => (
                  <option key={subj.subject_id} value={subj.subject_id}>
                    {subj.subject_name} {subj.is_required ? '(Required)' : '(Optional)'}
                  </option>
                ))}
              </select>
              {classSubjects.length === 0 && selectedClass && (
                <div className="subject-warning">
                  <AlertCircle size={14} />
                  <span>No subjects assigned to this class. Please assign subjects first.</span>
                </div>
              )}
            </div>
          </div>
          ...
          {selectedClass && classSubjects.length > 0 && (
            <div className="class-subjects-summary">
              <div className="summary-header">
                <BookOpen size={16} />
                <span>Subjects in {getClassName()}:</span>
              </div>
