...
//             {/* Machine Fingerprint */}
//            {/* Machine Fingerprint - Modified JSX */}
// <div className="mb-4 machine-fingerprint-section">
...
//     </button>
//   </div>
//   <div className="input-group machine-fingerprint-wrapper">
//     <span className="input-group-text">
//       <Fingerprint size={16} />
//     </span>
//     <input
//       id="machine-fingerprint-input"
//       type="text"
//       className="form-control font-monospace fingerprint-blurred"
...
//     <button
//       type="button"
//       className="btn btn-outline-secondary fingerprint-copy-btn"
...
//     <span>🔒 </span>
//     Machine fingerprint is protected and cannot be viewed or copied
//   </small> */}
// </div>
...
export default function ActivationPage({ onActivated, onBack, isOnline }) {
  ...
  const handleActivate = async (e) => {
    ...
    if (!activationCode.trim()) {
      setNotification({ message: "Enter activation code", type: "error" });
      return;
    }
    if (!schoolName.trim()) {
      setNotification({ message: "Enter school name", type: "error" });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => onActivated(), result.already_activated ? 500 : 2000);
      } else {
        setNotification({ message: result.message || "Activation failed", type: "error" });
      }
    } catch (error) {
      setNotification({ message: error.message || "Failed to connect", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const showRequestInfo = () => {
    setHelperContent({
      ...
      children: (
        <div>
          <p><strong>Send to vendor:</strong></p>
          <div className="card bg-light mb-3">
            <div className="card-body">
              <pre className="mb-0 small">
{`School: ${schoolName || "Your School"}
Fingerprint: ${machineId || "Loading..."}`}
              </pre>
            </div>
          </div>
          <p><strong>Contact:</strong> support@yourschoolsoftware.com</p>
        </div>
      )
    });
    setShowHelper(true);
  ...
  const showActivationInfo = () => {
    setHelperContent({
      ...
      children: (
        <div>
          <ul className="mb-0">
            <li>One license per computer</li>
            <li>Works offline after activation</li>
  ...
  return (
    <div className="app app-login p-0">
      ...
      <PageHelper open={showHelper} title={helperContent.title} onClose={() => setShowHelper(false)}>
        {helperContent.children}
      </PageHelper>
      ...
      <div className="row g-0 app-auth-wrapper">
        ...
        <div className="col-12 col-md-7 col-lg-6 auth-main-col p-4">
          <div className="app-auth-body mx-auto" style={{ maxWidth: '400px' }}>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <button className="btn btn-outline-secondary btn-sm" onClick={onBack} disabled={loading}>
                <ArrowLeft size={16} />
              </button>
              <h3 className="mb-0">Activation</h3>
              <button className="btn btn-outline-info btn-sm" onClick={showActivationInfo}>
                <Info size={16} />
              </button>
            </div>
            ...
            {/* Machine Fingerprint */}
            <div className="machine-fingerprint-section mb-2">
              <div className="input-group">
                <span className="input-group-text"><Fingerprint size={16} /></span>
                <input
                  type="text"
                  className="form-control fingerprint-blurred"
                  value={machineId || "Loading..."}
                  readOnly
                  disabled
                  style={{
              ...
              <div className="text-end mt-1">
                <button type="button" className="btn btn-link btn-sm p-0" onClick={showRequestInfo}>
                  <Send size={12} className="me-1" /> Request Code
                </button>
              </div>
            </div>
            ...
            <div className="input-group mb-3">
              <span className="input-group-text"><Key size={16} /></span>
              <input
                type="text"
                className="form-control text-uppercase"
            ...
            <button 
              ...
              disabled={loading || !activationCode.trim() || !schoolName.trim()}
            >
              {loading ? <span className="spinner-border spinner-border-sm" /> : "Activate"}
            </button>
          </div>
        </div>
        ...
        <div className="col-12 col-md-5 col-lg-6 auth-background-col text-white d-flex align-items-center justify-content-center">
          <div className="text-center p-4">
            <Fingerprint size={64} className="mb-3 opacity-75" />
            <h3>Activation</h3>
            <div className="mt-4">
              {[
                { num: 1, label: "Request", active: true },
                { num: 2, label: "Receive", active: true },
                { num: 3, label: "Activate", active: !!activationCode }
              ].map((item) => (
                <div key={item.num} className="d-flex align-items-center gap-3 mb-3">
