...
async def get_filter_options(
    ...
    """Get available filter options for dropdowns, optionally filtered by class"""
    logger.info(f"GET /api/subject-results/options - class_id={class_id}")
    conn = None
    ...
    try:
        ...
        # Get subjects - filter by class if class_id is provided
        subjects = []
        if class_id:
            # Get the class info to find its academic year
            cursor.execute("SELECT academic_year_id FROM classes WHERE id = ?", (class_id,))
            class_info = cursor.fetchone()
            academic_year_id = class_info['academic_year_id'] if class_info else None
            
            if academic_year_id:
                # Get subjects assigned to this class
                subject_ids = get_class_subjects(cursor, class_id, academic_year_id)
                if subject_ids:
            ...
            # If no subjects found for class, return empty list
            if not subjects:
                logger.warning(f"No subjects found for class {class_id}")
        else:
            # Return all subjects
            cursor.execute("SELECT id, name FROM subjects ORDER BY name")
            subjects = [{"id": row['id'], "name": row['name']} for row in cursor.fetchall()]
