...
def get_student_info(cursor, student_id: int) -> Dict:
    """Get student information"""
    cursor.execute("""
        SELECT s.id, s.student_number, s.class_id, s.section_id, s.academic_year_id,
               p.first_name, p.last_name, p.other_names, p.email,
               c.class_name, sec.section_name
        FROM students s
        JOIN person_details p ON s.person_id = p.id
        JOIN classes c ON s.class_id = c.id
...
def get_class_subjects(cursor, class_id: int, academic_year_id: int) -> List[int]:
    ...
    # If still no subjects, get all subjects as fallback
...
def get_subject_results_for_student(cursor, student_id: int, term_id: int, class_subject_ids: List[int]) -> List[Dict]:
    """Get subject results for a student, filtered by class subjects"""
    results = []
    
    if not class_subject_ids:
        logger.warning(f"No class subjects found, returning empty results")
        return results
    ...
    query = f"""
        ...
        ORDER BY s.name
    """
    cursor.execute(query, [student_id, term_id] + class_subject_ids)
    rows = cursor.fetchall()
    ...
    logger.info(f"Returning {len(results)} subject results for student {student_id}")
    return results
...
async def generate_report_card(
    ...
    try:
        ...
        logger.info(f"Student {student_info['name']} - Class ID: {class_id}, Academic Year ID: {student_info['academic_year_id']}")
        
        # Get subjects assigned to this student's class (with fallback for academic year mismatch)
        class_subject_ids = get_class_subjects(cursor, class_id, student_info['academic_year_id'])
        logger.info(f"Class subjects for class {class_id}: {len(class_subject_ids)} subjects")
        ...
        # Get subject results - filtered by class subjects only
        subject_results = get_subject_results_for_student(cursor, student_id, term_id, class_subject_ids)
        logger.info(f"Filtered subject results count: {len(subject_results)}")
        ...
        # Calculate totals - only for filtered subjects
        total_score = sum(r['score'] for r in subject_results) if subject_results else 0
        max_possible = len(subject_results) * 100
